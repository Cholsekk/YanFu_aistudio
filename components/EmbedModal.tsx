  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="嵌入到网站" maxWidth="max-w-3xl" zIndex="z-[1050]">
      <div className="p-8 space-y-8">
        {/* Header with Icon */}
        <div className="flex items-center gap-4 p-4 bg-primary-50 text-primary-700 rounded-2xl border border-primary-100">
          <div className="p-2 bg-white rounded-full shadow-sm">
            <Info className="w-6 h-6 text-primary-600" />
          </div>
          <div>
            <h3 className="font-bold text-primary-900">快速嵌入 AI 助手</h3>
            <p className="text-sm text-primary-700/80">选择一种方式，将聊天应用集成到你的网站中。</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <button 
            ...
            className={`p-6 border-2 rounded-2xl transition-all flex flex-col items-center gap-4 ${embedType === 'iframe' ? 'border-primary-500 bg-primary-50 shadow-lg shadow-primary-100' : 'border-gray-100 hover:border-gray-200 bg-white'}`}
          >
            <Monitor className={`w-10 h-10 ${embedType === 'iframe' ? 'text-primary-600' : 'text-gray-400'}`} />
