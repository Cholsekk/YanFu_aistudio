  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="WebApp 设置" maxWidth="max-w-2xl">
      <div className="flex">
        ...
        <div className="w-40 border-r border-gray-100 p-3 space-y-1">
          <button 
            ...
            className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'basic' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'}`}
          >
            <Settings className="w-4 h-4" /> 基础设置
          </button>
          <button 
            onClick={() => setActiveTab('advanced')}
            className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === 'advanced' ? 'bg-primary-50 text-primary-700' : 'text-gray-600 hover:bg-gray-50'}`}
          >
          ...
          {activeTab === 'basic' ? (
            <>
              <div className="space-y-1">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    className="w-10 h-10 bg-primary-50 rounded-xl flex items-center justify-center text-primary-600 shadow-sm border border-primary-100 hover:bg-primary-100 transition-colors overflow-hidden"
                  >
                    {iconType === 'image' ? (
                  ...
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="flex-1 p-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none text-sm" />
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-900">WebApp 描述</label>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="请输入 WebApp 的描述" className="w-full p-2 bg-gray-50 border border-gray-200 rounded-xl h-20 focus:ring-2 focus:ring-primary-500 outline-none resize-none text-sm" />
              </div>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                ...
                <button onClick={() => setUseCustomIcon(!useCustomIcon)} className={`w-10 h-5 rounded-full transition-colors ${useCustomIcon ? 'bg-primary-600' : 'bg-gray-300'}`}>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <select 
                    ...
                    className="w-full p-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none text-sm appearance-none cursor-pointer focus:ring-2 focus:ring-primary-500 pr-8"
                  >
            ...
            <div className="space-y-4 animate-in fade-in duration-300">
              <div className="space-y-1">
                ...
                <input type="text" value={copyright} onChange={(e) => setCopyright(e.target.value)} placeholder="请输入作者或组织名称" className="w-full p-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none text-sm" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-900">隐私政策</label>
                <input type="text" value={privacyPolicy} onChange={(e) => setPrivacyPolicy(e.target.value)} placeholder="请输入隐私政策链接" className="w-full p-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none text-sm" />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-900">自定义免责声明</label>
                <textarea value={customDisclaimer} onChange={(e) => setCustomDisclaimer(e.target.value)} placeholder="请输入免责声明" className="w-full p-2 bg-gray-50 border border-gray-200 rounded-xl h-20 focus:ring-2 focus:ring-primary-500 outline-none resize-none text-sm" />
              </div>
            </div>
          )}
        </div>
      ...
      <div className="flex justify-end gap-2 p-4 border-t border-gray-100 bg-gray-50/50 rounded-b-2xl">
        ...
        <button onClick={handleSave} className="px-4 py-2 text-xs font-semibold text-white bg-primary-600 hover:bg-primary-700 rounded-lg shadow-md shadow-primary-200 transition-all">保存</button>
      </div>
