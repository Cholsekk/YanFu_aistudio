        ...
        return [
          {
            ...
            target: () => document.querySelector('#tour-filter-search .flex-wrap') as HTMLElement,
          },
          {
            title: '精准搜索与高级视图控制',
            description: '如果分类过滤还不够，在此区域您可以输入关键词进行应用名搜索、通过高级筛选根据更新时间或名称进行列表排序，甚至自由切换网格与列表两种展现视图，让查找变得极为高效。',
            target: () => document.querySelector('#tour-filter-search .flex.items-center.gap-3:last-child') as HTMLElement,
          },
          {
            title: '从零新建空白应用',
          ...
          },
          {
            ...
            target: () => {
              ...
              return firstCard ? firstCard as HTMLElement : null;
            },
          },
          {
            title: '应用标签与分类管理',
            ...
            target: () => {
              ...
              return (tagsArea || firstCard) as HTMLElement;
            },
          },
          {
            title: '快捷操作与应用转换',
            ...
            target: () => {
              ...
              return btn || firstCard as HTMLElement;
            },
          }
        ];
      case 'tools':
        const isBuiltin = document.getElementById('tour-tab-builtin')?.classList.contains('bg-blue-600');
        const isWorkflow = document.getElementById('tour-tab-workflow')?.classList.contains('bg-blue-600');
        const isMcp = document.getElementById('tour-tab-mcp')?.classList.contains('bg-blue-600');
        const isSkills = document.getElementById('tour-tab-skills')?.classList.contains('bg-blue-600');
          ...
          return [
            {
              ...
              target: () => document.getElementById('tour-tab-workflow') as HTMLElement,
            },
            {
              title: '工具搜索',
              description: '可以通过输入工具名称搜索，快速定位特定的工作流工具。',
              target: () => document.querySelector('input[name="tool-search"]')?.parentElement as HTMLElement,
            },
            {
              title: '工具分类筛选',
              description: '在右侧下拉菜单中，可以根据工具分类标签进一步缩小查找范围。',
              target: () => document.getElementById('tour-tool-filter-btn') as HTMLElement,
            }
          ];
        }
        ...
        return [
          {
            ...
            target: () => document.getElementById('tour-tab-builtin') as HTMLElement,
          },
          {
            title: '工具搜索筛选',
        ...
        return [
          {
            ...
            target: () => document.querySelector('.bg-indigo-50\\/50') as HTMLElement,
          },
          {
            title: '配置模型供应商',
            description: '展示所有的受支持的模型供应商。必须首先提供有效的供应商 API Key。',
            target: () => document.querySelector('.bg-white.rounded-xl.shadow-sm') as HTMLElement,
          },
          {
            title: '激活具体模型',
            description: '点击任意一个供应商，通过表单进行密钥验证，状态变为绿色即代表可用。',
            target: () => document.querySelector('.group.cursor-pointer') as HTMLElement,
          }
        ];
      case 'tasks':
        return [
          {
            ...
            target: () => document.getElementById('tour-create-task') as HTMLElement,
          },
          {
            title: '任务搜索与过滤',
            description: '在这里搜索您的所有定时任务。',
            target: () => document.getElementById('tour-search-task') as HTMLElement,
          },
          {
            title: '任务管理与日志',
            description: '对于每一个任务，您可以查看它的执行日志、编辑规则、以及开启或暂停执行任务。',
            target: () => document.querySelector('.bg-white.rounded-xl.shadow-sm.border') as HTMLElement,
          }
        ];
      default:
        return [];
